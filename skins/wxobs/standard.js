...
function choose_history(id) {
  choose_div('history', id, ['day', 'week', 'month', 'year', 'dayrain', 'weekrain']);
...
function openNOAAFile(date) {
  if (date.match(/^\d\d\d\d/)) {
    window.location = "/weewx/NOAA/NOAA-" + date + ".txt";
  }
}

function openTabularFile(date) {
  if (date.match(/^\d\d\d\d/)) {
    window.location = "/weewx/tabular.html?report=NOAA/NOAA-" + date + ".txt";
  }
}
//function openNOAAFile(date) {
